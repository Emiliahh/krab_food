...
const CustomerPage = () => {
  ...
  const [customers, setCustomers] = useState<UserManager[]>([]);
  const [filteredCustomers, setFilteredCustomers] = useState<UserManager[]>([]);
  ...
  const handleDelete = async (id: string) => {
    const confirm = window.confirm("Bạn có chắc chắn muốn xóa khách hàng này?");
    if (!confirm) return;
    ...
    <div className="p-8">
      ...
      <AddUser
        ...
        onClose={(newUser?: User) => {
          setOpenDialog(false);
          if (newUser) {
            const userWithStatus: UserManager = {
              ...newUser,
              status: newUser.status || "active",
              isBanned: newUser.status === "blocked",
            };
            const updatedList: UserManager[] = [userWithStatus, ...customers];
